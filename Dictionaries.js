...
function infDatabase(numRange, constraint, value) {
    const validConstraints = {
        "equals to": (x) => x === value,
        "higher than": (x) => x > value,
        "lower than": (x) => x < value,
        "higher and equals to": (x) => x >= value,
        "lower and equals to": (x) => x <= value
    };
    
    if (!(constraint in validConstraints)) {
    ...
    for (let n = a; n <= b; n++) {
        if (validConstraints[constraint](A001055[n])) {
            filteredNumbers.push(n);
        }
    }
    
    return [filteredNumbers.length, filteredNumbers];
}